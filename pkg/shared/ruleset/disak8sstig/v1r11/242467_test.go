...
var _ = Describe("#242467", func() {
  ...
  },
  {
	...
	BeforeEach(func() {
		...
		dikiPod.Name = fmt.Sprintf("diki-%s-%s", v1r11.ID242467, "aaaaaaaaaa")
	...
	DescribeTable("Run cases",
		...
		Entry("should return passed checkResults when files have expected permissions",
			...
			[]rule.CheckResult{
				...
				rule.PassedCheckResult("File has expected permissions", rule.NewTarget("name", "1-pod", "namespace", "foo", "containerName", "test", "kind", "pod", "details", "fileName: /destination/bar/file2.key, permissions: 400")),
