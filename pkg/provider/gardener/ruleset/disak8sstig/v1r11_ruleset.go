...
func (r *Ruleset) registerV1R11Rules(ruleOptions map[string]config.RuleOptionsConfig) error { //nolint:unused // TODO: add to FromGenericConfig
	...
	rules := []rule.Rule{
		...
		rule.NewSkipRule(
			...
			"Kubernetes conf files must be owned by root (MEDIUM 242446)",
		...
		rule.NewSkipRule(
			...
			"Kubernetes component PKI must be owned by root (MEDIUM 242451)",
			`Rule implemented by "node-files" for correctness, consistency, deduplication, reliability, and performance reasons.`,
			rule.Skipped,
		),
		rule.NewSkipRule(
		...
		rule.NewSkipRule(
			...
			"Kubernetes admin.conf must have file permissions set to 644 or more restrictive (MEDIUM 242460)",
			`Gardener does not use "kubeadm" and also does not store any "main config" anywhere in seed or shoot (flow/component logic built-in/in-code).`,
			rule.Skipped,
		),
