	...
	DescribeTable("Run cases",
		func(executeReturnString [][]string, executeReturnError [][]error, expectedCheckResults []rule.CheckResult) {
			alwaysExpectedCheckResults := []rule.CheckResult{
				rule.FailedCheckResult("Option authentication.x509.clientCAFile is empty.", rule.NewTarget("cluster", "shoot", "kind", "node", "name", "node1")),
				rule.PassedCheckResult("Option authentication.x509.clientCAFile set.", rule.NewTarget("cluster", "shoot", "kind", "node", "name", "node2")),
				rule.WarningCheckResult("Node is not in Ready state.", rule.NewTarget("cluster", "shoot", "kind", "node", "name", "node3")),
				rule.FailedCheckResult("Option authentication.x509.clientCAFile not set.", rule.NewTarget("cluster", "shoot", "kind", "node", "name", "node4")),
				rule.FailedCheckResult("Option authentication.x509.clientCAFile is empty.", rule.NewTarget("cluster", "shoot", "kind", "node", "name", "node5")),
			}
