...
func (r *Rule242377) Run(ctx context.Context) (rule.RuleResult, error) {
	...
	target := gardener.NewTarget("cluster", "seed", "name", ksName, "namespace", r.Namespace, "kind", "deployment")

	optSlice, err := utils.GetCommandOptionFromDeployment(ctx, r.Client, ksName, ksName, r.Namespace, option)
	if err != nil {
