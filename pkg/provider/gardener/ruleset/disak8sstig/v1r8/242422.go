...
func (r *Rule242422) Run(ctx context.Context) (rule.RuleResult, error) {
	...
	tlsCertFileOptionSlice, err := utils.GetCommandOptionFromDeployment(ctx, r.Client, kapiName, kapiName, r.Namespace, certOptName)
	if err != nil {
	...
	tlsPrivateKeyFileOptionSlice, err := utils.GetCommandOptionFromDeployment(ctx, r.Client, kapiName, kapiName, r.Namespace, keyOptName)
	if err != nil {
		return rule.SingleCheckResult(r, rule.ErroredCheckResult(err.Error(), target)), nil
	}
